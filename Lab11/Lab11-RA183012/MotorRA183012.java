...
* RA: 183012
*
* Completude: 10 (De 0 a 10, escreva quanto você acha que "completou" das atividades propostas)
* Escreva aqui (sucintamente) os tratamentos que você implementou:
 *  Acredito que tenha implementado tudo que foi solicitado pelo enunciado do laboratorio:
 *
  *  No case para jogadas de ATAQUE, verifico se a cartaJogada esta contida nos lacaios da mesa do jogador, depois verifico se o lacaio
    *  em questao nao foi baixado no turno no qual estamos. Caso a cartaAlvo seja um lacaio, verificamos se a mesma esta
    *  na mesa do jogadorOponente, caso nao seja um lacaio e nem o heroi, temos um erro. Alem disso, verificamos se o lacaio
    *  que ira atacar ja nao atacou em algum momento.
    *
  *  No case para jogadas de MAGIA, verifico se a cartaJogada eh do tipo magia, se ela esta contida na mão do jogador, se o
    * jogador possui mana para jogar a carta. Caso a cartaJogada seja do tipo ALVO, verificamos se a cartaAlvo eh valida
    * (se o lacaio esta na mesa do oponente ou se eh o heroi). Caso seja do tipo BUFF tambem verificamos se a cartaAlvo eh
    * valida (um lacaio da mesa do jogador). Caso seja do tipo AREA, nao eh preciso fazer verificacao alguma, apenas atacar
    * todos os lacaios.
    *
  *  No case para jogadas de PODER, verificamos se o poderHeroico ja foi usado, caso a cartaAlvo seja nula, so precisamos
    * verificar se o jogador possui mana para realizar a jogada. Caso a cartaAlvo seja um lacaio, verificamos se o mesmo
    * esta na mesa do oponente e o efetuador da jogada esta na mesa verificamos se existe mana.
    *
                ...
                if (cartaJogada instanceof CartaLacaio) {
                    if (lacaios.contains(cartaJogada)) {
                        if (((CartaLacaio)cartaJogada).getTurno() != turno) {
                            ...
                            String errorMessage = "O lacaio de id "+cartaJogada.getID()+ " desceu nesse turno";
                            imprimir(errorMessage);
                            ...
                            if (cartaJogada.getMana() <= manaJogador1) {
                                ...
                                if (((CartaMagia) cartaJogada).getMagiaTipo() == TipoMagia.ALVO) {
                                    ...
                                    if (cartaAlvo == null) {
                                        ...
                                        imprimir("A carta Magia Area nao deve possuir um alvo em especifico");
                                    }
                                }
                            } else {
